...
runoff_day[, season := factor(season, levels = c('winter', 'spring', 'summer', 'autumn'))] #with factor we defined the order in which we want the categories. If not, it does it automatically alphabetically
...
runoff_summary <- runoff_summary[order(-altitude)] # the - symbol means in decreasing order
